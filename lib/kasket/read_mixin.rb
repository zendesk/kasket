module Kasket
  module ReadMixin
    def find_by_sql(*args)
      ...
      if query && has_kasket_index_on?(query[:index])
        if query[:key].is_a?(Array)
          find_by_sql_on_id_array(query[:key])
        else
          if value = Kasket.cache.read(query[:key])
            if value.is_a?(Array)
              filter_pending_records(find_by_sql_on_id_array(value))
            else
              filter_pending_records(Array.wrap(value).collect { |record| instantiate(record.dup) })
            end
          else
            store_in_kasket(query[:key], super(*args))
          end
        end
      else
        super(*args)
    ...
    def find_by_sql_on_id_array(keys)
